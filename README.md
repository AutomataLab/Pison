...
- `static Bitmap* construct(Record* record, int thread_num = 1, int level = MAX_LEVEL)`: constructs leveled bitmaps for a single record in parallel (indicated by `thread_num`); bitmap indices can be created based on the maximum level (starting from 0) of given queries (indicated by `level`).
